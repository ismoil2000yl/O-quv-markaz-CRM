...
const index = () => {
  ...
  const getData = async () => {
    ...
    const newData = data?.data?.filter(item => item?.status?.includes("administrator_user"))
    setData(newData)
  }
    ...
    <div className='w-full h-[72vh] px-4 py-2'>
      <div>
        <Breadcrumb>
          ...
          <Breadcrumb.Item className='text-[16px] text-[#002B48] font-medium'>Administrator</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      <div className='flex items-center justify-between mt-4'>
        <h1 className='text-[26px] text-[#002B48]'>Kurs administratori</h1>
        <Button
          type='ghost'
          ...
          <table className='w-full text-left border-t border-solid border-[#F2F2FD]'>
            <thead className='bg-[#25B7D3] text-white text-[14px] uppercase'>
              <tr className='border border-solid border-[#F2F2FD]'>
                ...
                <th className='min-w-[250px] p-4'>Administrator F.I.Sh</th>
                    ...
                    <tr key={item?.id} className='border border-solid border-[#F2F2FD]'>
                      ...
                      <td className='p-4 w-[110px]'>
                        <div className='flex items-center justify-center gap-4'>
                          <Button onClick={() => navigate(`/xodimlar/administrator/${item?.id}`)}>
                            <EyeOutlined />
                          </Button>
                          <Button onClick={() => navigate(`/xodimlar/administrator/edit/${item?.id}`)}>
                            <EditOutlined />
                          </Button>
                        </div>
                      </td>
