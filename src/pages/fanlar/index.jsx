...
import IconBook from 'assets/images/png/book.png'
...
const index = () => {
    ...
    const deleteConfirm = (id) => {
        deletedHandler({
            ...
            onSuccess: () => {
                ...
                messageApi.open({
                    type: 'success',
                    content: "Fan o'chirib yuborildi"
                })
            },
            onError: () => {
                messageApi.open({
                    type: 'error',
                    content: "Fan o'chirilmadi!"
                })
            }
        })
    };
    ...
    return (
        <div className='w-full h-[85vh] px-4 py-2 overflow-y-auto'>
            {contextHolder}
            <div>
                <Breadcrumb>
                    ...
                    <Breadcrumb.Item className='text-[16px] text-[#787F95] font-medium'>Fanlar</Breadcrumb.Item>
                </Breadcrumb>
            </div>
            <div className='flex items-center justify-between mt-4'>
                <h1 className='text-[26px] text-[#002B48]'>Fanlar</h1>
                ...
                <Create modalData={modalData} setModalData={setModalData} getData={getData} chegirma={chegirma} />
            </div>
                            ...
                            <table className='min-w-[700px] w-full text-left border-t border-solid border-[#F2F2FD]'>
                                <thead className='bg-[#25B7D3] text-white text-[14px] uppercase'>
                                    <tr className='border border-solid border-[#F2F2FD]'>
                                        ...
                                        <th className='min-w-[130px] p-4'>Kurs narxi</th>
                                        <th className='min-w-[130px] p-4'>Chegirmasi</th>
                                ...
                                <tbody className='bg-white text-[#002B48] text-[16px]'>
                                    <tr>
                                        ...
                                        <td className='p-4'>
                                            <Input type='number' onChange={(e) => setNarxi(e.target.value)} prefix={<SearchOutlined />} className='shadow-md' placeholder='Izlash...' />
                                        </td>
                                        <td></td>
                                        <td></td>
                                    </tr>
                                    {
                                        data?.filter(item =>
                                            ...
                                            item?.price?.toLowerCase().includes(narxi.toLowerCase())
                                        )?.map((item, index) => {
                                            return (
                                                <tr key={item?.id} className='border border-solid border-[#F2F2FD]'>
                                                    ...
                                                    <td className='p-4 min-w-[130px]'>{parseInt(item?.price).toLocaleString("ru-RU")} UZS</td>
                                                    <td>
                                                        {/* {item?.discount ? ` % chegirma` : ""} */}
                                                        <span className='text-[red] text-[20px]'>
                                                            {chegirma?.map?.((i) => i.id === item?.discount && parseInt(i.discount_percent) + " %")}
                                                        </span>
                                                    </td>
                                    ...
                                    data?.filter(item =>
                                        ...
                                        return (
                                            <div key={item?.id} className='w-[300px] overflow-hidden h-[230px] rounded-[15px] bg-gradient-to-b from-custom-gradient-start via-custom-gradient-mid to-custom-gradient-end shadow-custom p-2 relative'>
                                                {
                                                    item?.discount ?
                                                        <span className='w-full absolute h-[40px] rotate-45 bg-red-600 top-0 right-[-4rem] text-white text-[18px] pr-[2.5rem] pt-[0.5rem] text-end'>
                                                            {chegirma?.map?.((i) => i.id === item?.discount && parseInt(i.discount_percent) + " % chegirma")}
                                                        </span>
                                                        : null
                                                }
                                                <div className='w-full h-full flex flex-col items-center justify-center gap-[5px]'>
                                                    ...
                                                    <h3 className='text-white font-bold p-0 m-0'>{parseInt(item?.price).toLocaleString("ru-RU")} UZS</h3>
